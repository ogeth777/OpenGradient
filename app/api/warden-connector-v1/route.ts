...
export async function POST(req: Request) {
  try {
    ...
    if (typeof prompt === 'object' && prompt !== null && !Array.isArray(prompt)) {
        if (prompt.messages && Array.isArray(prompt.messages)) {
            ...
            if (envKey) {
                ...
                if (headerKey !== envKey) {
                     console.log("Blocked unauthorized request to /api/warden-connector-v1");
    ...
    console.log("Received prompt (connector v1):", prompt);
...
export async function GET() {
  const res = NextResponse.json({ status: "TERMINAL AI CONNECTOR V1 ONLINE", timestamp: Date.now() });
  return setCorsHeaders(res);
}
